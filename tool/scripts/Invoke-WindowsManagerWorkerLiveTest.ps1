...
if (-not $AllowGameLaunch) {
    throw 'worker live game launch requires -AllowGameLaunch'
}

if (-not $IsWindows -or [Runtime.InteropServices.RuntimeInformation]::OSArchitecture -ne
    [Runtime.InteropServices.Architecture]::X64) {
    throw 'Windows manager worker live test requires Windows x64.'
}
...
if (-not $runtimeItem.PSIsContainer) {
    throw 'Windows manager worker live test requires a runtime directory.'
}
...
if ([string]::IsNullOrWhiteSpace($runtimeDriveRoot)) {
    throw 'Windows manager worker live test requires a fixed local runtime directory.'
}
...
if ($preflight.status -ne 'verified' -or
    ...
    throw 'Windows manager worker live exact-runtime preflight did not match the pinned tuple.'
}
...
$liveTest = 'manager::worker::windows_live_runtime_tests::exact_runtime_runs_through_public_manager_worker'
...
Write-Output 'PASS: Windows manager worker live test'
